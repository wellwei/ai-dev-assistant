...
Do not directly trust memory, summaries, or symbol names when the current source can be inspected. If the evidence conflicts, say so in the final English answer and identify which current files should be checked next.
...
5. Run `scripts/ask_project.py` when a synthesized English agent-facing answer or saved `research_notes` entry is useful.
6. Final answer should be English and agent-readable. Separate evidence types when relevant:
...
3. Run `scripts/ask_project.py --output json` when the assistant graph's request classification, `approval_required`, related paths, open questions, or English synthesis are useful.
