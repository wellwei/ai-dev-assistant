...
def _search(db_path: Path, query: str, limit: int) -> list[dict[str, Any]]:
    try:
        return hybrid_search_project(db_path, query, limit=limit, initialize_schema=False)
    except Exception:
        try:
            return search_project_index(db_path, query, limit=limit, initialize_schema=False)
        except Exception:
            return []
