...
def test_search_project_cli_does_not_create_schema_for_legacy_database(tmp_path):
    db_path = tmp_path / "project_index.sqlite"
    with sqlite3.connect(db_path) as conn:
    ...
    result = _run_cli("--db", str(db_path), "--query", "legacy", "--output", "json")
    ...
    with sqlite3.connect(db_path) as conn:
        after_tables = conn.execute("SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name").fetchall()
