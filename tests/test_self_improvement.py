...
def test_draft_research_memory_proposals_does_not_treat_confirmed_chinese_notes_as_uncertain(tmp_path):
    db_path = tmp_path / "project_index.sqlite"
    repo = ProjectIndexRepository(db_path)
    repo.init()
    first = repo.insert_research_note(
        ResearchNote(
            thread_id="thread-confirmed-gameplay-1",
            request_type="requirement_research",
            question="移动位置同步逻辑在哪里？",
            answer_summary="已确认 movement position sync implementation is indexed with evidence.",
            related_paths=json.dumps(["src/rtb_proc/character/character_move.cpp"]),
            open_questions=json.dumps([]),
            project_root="/tmp/project",
            internal_memory_summary="已确认移动同步实现路径，no retrieval gap remains.",
            user_answer_summary="已确认移动同步实现路径。",
            confidence="medium",
        )
    )
    repo.insert_research_note(
        ResearchNote(
            thread_id="thread-confirmed-gameplay-2",
            request_type="requirement_research",
            question="坐骑上马同步在哪里？",
            answer_summary="已确认 mount sync implementation is indexed with evidence.",
            related_paths=json.dumps(["src/rtb_proc/character/rtb_proc_character_horse.cpp"]),
            open_questions=json.dumps([]),
            project_root="/tmp/project",
            source_note_ids=json.dumps([first]),
            internal_memory_summary="已确认坐骑同步实现路径，confirmed evidence.",
            user_answer_summary="已确认坐骑同步实现路径。",
            confidence="medium",
        )
    )
    ...
    assert proposals == []
    assert repo.list_improvement_proposals(status="pending") == []
